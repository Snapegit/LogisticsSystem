	...
	<div>
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					<el-col :span="24">
						<el-form-item label="票据单号" prop="piaojudanhao">
							<el-input class="list_inp" v-model="form.piaojudanhao" placeholder="票据单号"
								 type="text" 								:readonly="!isAdd||disabledForm.piaojudanhao?true:false" />
						</el-form-item>
					</el-col>
					...
					<el-col :span="24">
						<el-form-item label="下单时间" prop="xiadanshijian">
							<el-input class="list_inp" v-model="form.xiadanshijian" placeholder="下单时间"
								 type="text" 								:readonly="!isAdd||disabledForm.xiadanshijian?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="24">
						<el-form-item label="大概距离" prop="dagaijuli">
							<el-input class="list_inp" v-model.number="form.dagaijuli" placeholder="大概距离"
								 type="number" 								:readonly="!isAdd||disabledForm.dagaijuli?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="24">
						<el-form-item label="货物运费" prop="huowuyunfei">
							<el-input class="list_inp" v-model.number="form.huowuyunfei" placeholder="货物运费"
								 type="number" 								:readonly="!isAdd||disabledForm.huowuyunfei?true:false" />
						</el-form-item>
					</el-col>
					...
					<el-col :span="24">
						<el-form-item label="司机账号" prop="sijizhanghao">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.sijizhanghao?true:false"
								v-model="form.sijizhanghao" 
								placeholder="请选择司机账号"
								@change="sijizhanghaoChange">
								<el-option v-for="(item,index) in sijizhanghaoLists" :label="item" :value="item">
								</el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="司机姓名" prop="sijixingming">
							<el-input class="list_inp" v-model="form.sijixingming" placeholder="司机姓名"
								 type="text" 								:readonly="!isAdd||disabledForm.sijixingming?true:false" />
						</el-form-item>
					</el-col>
					...
					<el-col :span="24">
						<el-form-item label="配车时间" prop="peicheshijian">
							<el-date-picker
								class="list_date"
								v-model="form.peicheshijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.peicheshijian?true:false"
								placeholder="请选择配车时间" />
						</el-form-item>
					</el-col>
				</el-row>
...
<script setup>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		resetForm()
			form.value.peicheshijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				if(x=='piaojudanhao'){
					form.value.piaojudanhao = row[x];
					disabledForm.value.piaojudanhao = true;
					continue;
				}
				if(x=='huowumingcheng'){
				...
				if(x=='dagaijuli'){
					form.value.dagaijuli = row[x];
					disabledForm.value.dagaijuli = true;
					continue;
				}
				if(x=='huowuyunfei'){
					form.value.huowuyunfei = row[x];
					disabledForm.value.huowuyunfei = true;
					continue;
				}
				if(x=='sijizhanghao'){
					form.value.sijizhanghao = row[x];
					disabledForm.value.sijizhanghao = true;
					continue;
				}
				if(x=='sijixingming'){
					form.value.sijixingming = row[x];
					disabledForm.value.sijixingming = true;
					continue;
				}
				if(x=='shoujihaoma'){
				...
				if(x=='peicheshijian'){
					form.value.peicheshijian = row[x];
					disabledForm.value.peicheshijian = true;
					continue;
				}
			}
			if(row){
	...
	const sijizhanghaoChange=()=>{
		context?.$http({
			url: `follow/siji/sijizhanghao?columnValue=` + form.value.sijizhanghao,
			method: 'get'
		}).then(res=>{
			if(res.data.data.sijixingming){
				form.value.sijixingming = res.data.data.sijixingming
			}
