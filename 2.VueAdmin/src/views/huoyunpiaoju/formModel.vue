				...
				<el-row>
					<el-col :span="24">
						<el-form-item label="票据单号" prop="piaojudanhao">
							<el-input class="list_inp" v-model="form.piaojudanhao" :readonly="true" placeholder="票据单号" />
						</el-form-item>
					</el-col>
					<el-col :span="24">
					...
					<el-col :span="24">
						<el-form-item label="票据状态" prop="piaojuzhuangtai">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.piaojuzhuangtai?true:false"
								v-model="form.piaojuzhuangtai" 
								placeholder="请选择票据状态"
								>
								<el-option v-for="(item,index) in piaojuzhuangtaiLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
...
<script setup>
	...
	const type = ref('')
	//票据状态列表
	const piaojuzhuangtaiLists = ref([])
	//methods
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='piaojuzhuangtai'){
					form.value.piaojuzhuangtai = row[x];
					disabledForm.value.piaojuzhuangtai = true;
					continue;
				}
			}
			if(row){
		...
		piaojuzhuangtaiLists.value = "未到货单,未到车辆".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
