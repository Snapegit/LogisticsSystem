		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="配车状态" prop="peichezhuangtai">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.peichezhuangtai?true:false"
								v-model="form.peichezhuangtai" 
								placeholder="请选择配车状态"
								>
								<el-option v-for="(item,index) in peichezhuangtaiLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
...
<script setup>
	...
	//配车状态列表
	const peichezhuangtaiLists = ref([])
	//methods
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='peichezhuangtai'){
					form.value.peichezhuangtai = row[x];
					disabledForm.value.peichezhuangtai = true;
					continue;
				}
				if(x=='crossuserid'){
			...
			form.value.peichezhuangtai='未配车'
			formVisible.value = true
		}
		...
		peichezhuangtaiLists.value = "已配车,未配车".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
